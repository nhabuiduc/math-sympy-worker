...
export class Order extends Pr2MItemBase {
    convert(obj: P2Pr.C<"Order">): Pr2M.CResult {
        const exp = this.main.c(obj.symbols[0]);
        ...
        if (runs.length == 1) {
            runsRs = blockBd.joinBlocks(this.main.m(runs[0].symbols), () => blockBd.compositeBlock("\\rightarrow"))
        } else {
            const forms = prTh.list(runs.map(r => r.symbols[0]));
            const tos = prTh.list(runs.map(r => r.symbols[1]));
            runsRs = blockBd.joinBlocks(this.main.m([forms, tos]), () => blockBd.compositeBlock("\\rightarrow"))
        }
